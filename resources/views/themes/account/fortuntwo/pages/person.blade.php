    ...
    <div class="miro-page">
        ...
        <div class="grid gap-6 lg:grid-cols-[.8fr_1.2fr]">
            <aside class="miro-card miro-card--pink p-6 sm:p-8">
                ...
                <h1 class="break-words text-center text-2xl font-medium tracking-tight text-[#21140F]" style="font-family: var(--fortun-display-font);">{{ $person->full_name ?: __('account.not_specified') }}</h1>

                @if($person->telegram_username)
                    <p class="mt-2 text-center text-sm text-[#8C776E]">{{ '@' . $person->telegram_username }}</p>
                ...
                @else
                    <p class="mt-7 text-center text-sm leading-6 text-[#21140F]/60">{{ __('account.profile.telegram_empty') }}</p>
                @endif
            </aside>
            ...
            <section class="space-y-6">
                <div class="miro-card p-6 sm:p-7">
                    ...
                    <p class="whitespace-pre-line text-sm leading-7 text-[#756158]">{{ $person->description ?: __('account.profile.description_empty') }}</p>
                </div>
                ...
                <div class="miro-card miro-card--coral p-6 sm:p-7">
                    ...
                    <p class="whitespace-pre-line text-sm leading-7 text-[#756158]">{{ $person->expectation ?: __('account.profile.expectation_empty') }}</p>
                </div>
            </section>
        </div>
    </div>
