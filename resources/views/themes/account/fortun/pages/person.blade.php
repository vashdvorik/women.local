    ...
    <div class="miro-page">
        ...
        <div class="grid gap-6 lg:grid-cols-[.8fr_1.2fr]">
            <aside class="miro-card miro-card--pink p-6 sm:p-8">
                ...
                <h1 class="break-words text-center text-2xl font-medium tracking-tight text-[#261153]">{{ $person->full_name ?: __('account.not_specified') }}</h1>

                @if($person->telegram_username)
                    <p class="mt-2 text-center text-sm text-[#706a79]">{{ '@' . $person->telegram_username }}</p>
                ...
                @else
                    <p class="mt-7 text-center text-sm leading-6 text-[#261153]/60">{{ __('account.profile.telegram_empty') }}</p>
                @endif
            </aside>
            ...
            <section class="space-y-6">
                <div class="miro-card p-6 sm:p-7">
                    ...
                    <p class="whitespace-pre-line text-sm leading-7 text-[#585364]">{{ $person->description ?: __('account.profile.description_empty') }}</p>
                </div>
