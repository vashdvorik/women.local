...
<div class="miro-page">
    ...
    <div class="grid gap-6 lg:grid-cols-[.78fr_1.22fr]">
        <aside class="miro-card miro-card--pink p-6">
            ...
            @else
                <p class="mt-3 text-sm text-[#261153]/60">{{ __('account.profile.telegram_empty') }}</p>
            @endif
        ...
        <section class="space-y-6">
            <div class="miro-card p-6 sm:p-7">
                <div class="miro-section-heading">
                    ...
                    <span class="rounded-full bg-[#e2f2f3] px-3 py-1 text-xs font-medium text-[#006a77]">{{ __('account.profile.public_label') }}</span>
                </div>
                <div class="space-y-5">
                    <div class="miro-card--soft rounded-2xl p-4">
                        <p class="miro-card__label">{{ __('account.profile.description') }}</p>
                        <p class="m-0 whitespace-pre-line text-sm leading-6 text-[#261153]">{{ $accountUser->description ?: __('account.profile.description_empty') }}</p>
                    </div>
                    <div class="miro-card--pink rounded-2xl p-4">
                        <p class="miro-card__label">{{ __('account.profile.expectation') }}</p>
                        <p class="m-0 whitespace-pre-line text-sm leading-6 text-[#261153]">{{ $accountUser->expectation ?: __('account.profile.expectation_empty') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="miro-card miro-card--teal flex items-start gap-4 p-6">
                ...
                <div>
                    <h2 class="m-0 text-base font-medium text-[#261153]">{{ __('account.profile.profile_tip_title') }}</h2>
                    <p class="mt-2 text-sm leading-6 text-[#006a77]">{{ __('account.profile.profile_tip_text') }}</p>
                </div>
            </div>
        </section>
    </div>
